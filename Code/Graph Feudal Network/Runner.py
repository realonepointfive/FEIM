...
class GFNRunner(object):
    ...
    def test(self):
        ...
        for epoch_training in range(4, 5):
            episode_end = round(float(self.episodes) / 10 * epoch_training)
            model_dir = self.model_dir.format(episode_end)
            print("Model test for GFN_{}.pt".format(episode_end))

            self.num_act_nodes = []
            self.msg = []

            policy_net_state_dict = torch.load(model_dir, map_location=torch.device('cpu'))
            self.policy_net.load_state_dict(policy_net_state_dict)
            self.policy_net.eval()

            start_time = time.time()
            for epoch_eval in range(9, 11):
                if epoch_eval == 9:
                    episode_start = 900
                else:
                    episode_start = round(float(self.episodes) / 10 * (epoch_eval-1))

                episode_end = round(float(self.episodes) / 10 * epoch_eval)

                for episode in range(episode_start, episode_end):
                    sg_id = episode
                    obs = self.env.reset('30%', sg_id)
                    terminated = False
                    episode_step = 0
                    ...
                    while(episode_step < self.episode_length):
                        ...
                        for step in range(1000):
                            ...
                            if a_values == None:
                                ...
                                action_probs = F.softmax(a_values, dim=1)
                                edge_indices = torch.argmax(action_probs)

                            obs, reward, terminated = self.env.step(g_nodes, edge_indices)
                            if terminated:
                                self.num_act_nodes.append(self.env.inf)
                            self.msg.append(self.env.ep_msg_num)

                        if episode % self.log_interval == 0:
                            print("\n Scenario {} Algo {} Exp {}, {}/{} episodes, inf {}, msg {}.\n"
                                    .format(self.all_args.scenario,
                                            self.algorithm_name,
                                            self.experiment_name,
                                            episode,
            ...
            print(np.sum(self.num_act_nodes))
            print(np.sum(self.msg))
            np.savetxt(str(self.save_dir) + '/inf.txt', np.array(self.num_act_nodes), fmt='%d')
            np.savetxt(str(self.save_dir) + '/msg.txt', np.array(self.msg), fmt='%d')