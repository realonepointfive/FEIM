...
def EventInfluenceSimulation(args, diff_g, rr, seeds):
        ...
        while(i<args.sim_num):
            ...
            while(len(cand_nodes)):
                g_edges = search_cand_edges('simulation', cand_nodes, diff_g, rr)
                edge_probs_list = [rr.edges[edge]['ppd'] for edge in g_edges]
                edge_probs_array = np.array(edge_probs_list)
                results = process_edges_batch_numba(g_edges, edge_probs_array, args.l)
